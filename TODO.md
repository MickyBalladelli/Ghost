...
- [ ] Add atomic writes: write to a temporary file, flush, then rename. Keep a backup and restore it when verification fails.
...
- [ ] Replace the raw `process.env` terminal environment with a masked, configurable allowlist. Never expose API keys or secret variables to the model.
- [ ] Add command timeout, output limit, cancellation, and process-tree cleanup for Windows, macOS, and Linux.
