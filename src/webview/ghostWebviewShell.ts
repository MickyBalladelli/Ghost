...
const createAppShell = (iconUri: string): HTMLDivElement => {
  ...
  const header = createElement('header', { className: 'header' }, [
    ...
    createElement('div', { className: 'header-actions' }, [
      button('history', '', { className: 'icon-button history-button', 'aria-haspopup': 'dialog', 'aria-label': 'History', 'data-tooltip': 'History' }, [svgIcon('M3 12a9 9 0 1 0 3-6.7M3 4v5h5M12 7v5l3 2')]),
      button('new-chat', '+', { className: 'icon-button', 'aria-label': 'New conversation', 'data-tooltip': 'New conversation' }),
      button('import', '⇩', { className: 'icon-button', 'aria-label': 'Import conversations', 'data-tooltip': 'Import conversations' }),
      button('export', '⇧', { className: 'icon-button', 'aria-label': 'Export conversations', 'data-tooltip': 'Export conversations' }),
      button('reset', '', { className: 'icon-button danger-button', 'aria-label': 'Delete all conversation history and preferences', 'data-tooltip': 'Delete all conversation history and preferences' }, [svgIcon('M6 7h12l-1 13H7L6 7Zm3-3h6l1 2H8l1-2Zm-1 7v6m4-6v6m4-6v6')])
    ])
  ])
  ...
  const controls = createElement('section', { className: 'control-strip', 'aria-label': 'Prompt controls' }, [
    button('toggle-provider-area', '', { className: 'provider-area-toggle', 'aria-label': 'Collapse provider controls', 'data-tooltip': 'Collapse provider controls', 'aria-controls': 'provider-area-content', 'aria-expanded': 'true' }, [svgIcon('M6 9l6 6 6-6')]),
    button('settings', '', { className: 'control-button settings-button', 'aria-haspopup': 'dialog', 'aria-label': 'Settings', 'data-tooltip': 'Settings' }, [svgIcon(settingsIconPath, '-1 -1 26 26')]),
    button('help', '?', { className: 'control-button help-button', 'aria-haspopup': 'dialog', 'aria-label': 'OpenCode setup help', 'data-tooltip': 'OpenCode setup help', 'data-provider-scope': 'opencode' }),
    providerAreaContent
  ])
  const composer = createElement('form', { className: 'composer', id: 'composer' }, [
    ...
    createElement('div', { className: 'composer-footer' }, [button('composer-hint', '', { className: 'icon-button composer-hint-button', 'aria-label': 'Enter to send · Shift+Enter for a new line · ↑/↓ prompt history · Cmd/Ctrl+N new conversation', 'data-tooltip': 'Enter to send · Shift+Enter for a new line · ↑/↓ prompt history · Cmd/Ctrl+N new conversation' }, [svgIcon('M12 8h.01M12 12v4M12 22a10 10 0 1 0 0-20 10 10 0 0 0 0 20Z')]), createElement('span', { className: 'composer-count', id: 'composer-count' }, ['0 chars · ~0 tokens']), createElement('span', { className: 'persistence-status', id: 'persistence-status', role: 'status', 'aria-live': 'polite', 'aria-atomic': 'true' }), button('stop', 'Stop', { className: 'stop-button', hidden: true }), select('mode', [option('agent', 'Agent'), option('plan', 'Plan'), option('ask', 'Ask'), option('edit', 'Edit'), option('explain', 'Explain')], { className: 'workflow-mode-select', 'aria-label': 'Workflow mode', title: 'Workflow mode' }), button('send', '', { className: 'composer-action-button', 'aria-label': 'Send', 'data-tooltip': 'Send' }, [svgIcon('M3 12h14M13 6l6 6-6 6')]), createElement('span', { className: 'prompt-history-actions', 'aria-label': 'Prompt history' }, [button('search-prompt-history', '', { className: 'secondary prompt-history-button composer-action-button', 'aria-label': 'Search prompt history', 'data-tooltip': 'Search prompt history' }, [svgIcon('M18 11a7 7 0 1 1-14 0 7 7 0 0 1 14 0ZM16 16l5 5')]), button('previous-prompt', '', { className: 'secondary prompt-history-button composer-action-button', 'aria-label': 'Previous prompt', 'data-tooltip': 'Previous prompt' }, [svgIcon('M12 18V6M6 12l6-6 6 6')]), button('next-prompt', '', { className: 'secondary prompt-history-button composer-action-button', 'aria-label': 'Next prompt', 'data-tooltip': 'Next prompt' }, [svgIcon('M12 6v12m-6-6 6 6 6-6')])])])
  ])
  ...
  const helpCommandRow = (labelText: string, command: string, note?: string): HTMLDivElement =>
    createElement('div', { className: 'help-command-row' }, [
      createElement('div', { className: 'help-command-text' }, [
        createElement('strong', {}, [labelText]),
        ...(note ? [createElement('small', {}, [note])] : []),
        createElement('code', {}, [command])
      ]),
      button('', 'Copy', { className: 'copy-button', 'data-copy-text': encodeURIComponent(command), 'aria-label': `Copy ${labelText} command` })
    ])
  const helpContent = createElement('div', { className: 'modal-scroll' }, [
    createElement('p', { className: 'modal-description' }, ['Install OpenCode, then start its headless server so Ghost can connect.']),
    createElement('div', { className: 'help-section' }, [
      createElement('h3', {}, ['Install OpenCode']),
      helpCommandRow('Script (Recommended)', 'curl -fsSL https://opencode.ai/install | bash'),
      helpCommandRow('Node.js package managers', 'npm install -g opencode-ai', 'Swap npm for bun, pnpm, or yarn.'),
      helpCommandRow('Homebrew (macOS / Linux)', 'brew install anomalyco/tap/opencode'),
      helpCommandRow('Windows (chocolatey)', 'choco install opencode', 'WSL recommended.'),
      helpCommandRow('Windows (scoop)', 'scoop install opencode', 'WSL recommended.')
    ]),
    createElement('div', { className: 'help-section' }, [
      createElement('h3', {}, ['Running opencode serve']),
      helpCommandRow('Default server', 'opencode serve', 'Starts a headless server listening on 127.0.0.1:4096.'),
      helpCommandRow('Custom port & host', 'opencode serve --port 8080 --hostname 0.0.0.0'),
      helpCommandRow('CORS allowed origins', 'opencode serve --cors http://localhost:5173'),
      helpCommandRow('Password authentication', 'OPENCODE_SERVER_PASSWORD=your-password opencode serve')
    ])
  ])
  const root = createElement('div', { className: 'app' }, [header, controls, chat, createSettingsModal(), modal('help-modal', 'OpenCode setup help', 'help-title', helpContent, button('', 'Close', { className: 'secondary', 'data-close-modal': 'help-modal' })), modal('tool-permissions-modal', 'Tool permissions', 'tool-permissions-title', toolsContent, button('', 'Done', { className: 'secondary', 'data-close-modal': 'tool-permissions-modal' })), modal('terminal-environment-permissions-modal', 'Terminal environment', 'terminal-environment-permissions-title', terminalContent, button('', 'Done', { className: 'secondary', 'data-close-modal': 'terminal-environment-permissions-modal' })), modal('privacy-modal', 'Ghost privacy', 'privacy-title', privacyContent, button('', 'Close', { className: 'secondary', 'data-close-modal': 'privacy-modal' })), modal('context-modal', 'Prompt context', 'context-title', contextContent, button('', 'Done', { className: 'secondary', 'data-close-modal': 'context-modal' })), modal('first-run-modal', 'Set up Ghost', 'first-run-title', setupContent, button('finish-first-run', 'Finish setup')), modal('history-modal', 'Conversation history', 'history-title', historyContent, createElement('span', {}, [button('new-history-chat', 'New conversation', { className: 'secondary' }), button('', 'Close', { className: 'secondary', 'data-close-modal': 'history-modal' })])), modal('delete-conversation-modal', 'Delete conversation', 'delete-conversation-title', deleteConversationContent, createElement('span', {}, [button('cancel-delete-conversation', 'Cancel', { className: 'secondary', 'data-close-modal': 'delete-conversation-modal' }), button('confirm-delete-conversation', 'Delete', { className: 'danger-button' })])), modal('prompt-history-modal', 'Prompt history', 'prompt-history-title', promptHistoryContent, button('', 'Close', { className: 'secondary', 'data-close-modal': 'prompt-history-modal' })), (() => {
